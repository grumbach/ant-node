...
/// Pick `k` distinct nonce-random leaf positions within the selected subtree.
///
/// Returned as indices into `path.real_leaf_count()` (0-based within the
/// subtree). Used for the real-bytes spot-check (ADR-0002). Deterministic from
/// the nonce so the auditor and any observer derive the same positions; the
/// responder cannot predict-and-fake only these because it must produce a
/// correct nonced hash for *every* returned leaf anyway — the spot-check just
/// bounds how many it can fake and still pass.
#[must_use]
pub fn select_spotcheck_indices(nonce: &[u8; 32], path: &SubtreePath, k: u32) -> Vec<u32> {
    ...
    // mod n; skip collisions. Bounded: k is small (default 8) and n > k.
    ...
    while u32::try_from(out.len()).unwrap_or(u32::MAX) < k {
        ...
        // Safety valve: with n > k this terminates quickly, but bound the loop.
...
fn fold_levels(mut level: Vec<[u8; 32]>, levels: u32) -> [u8; 32] {
    ...
    for _ in 0..levels {
        ...
        while i < level.len() {
            let left = level[i];
            // Missing right sibling → self-pair the last node, identical to
            // `build_next_level`. Within a selected block this happens only at
            // the tree's odd tail, so it matches the committed build exactly.
